...
fms = ['Bedrock Titan', 'Bedrock Claude 3.5 Sonnet v2', 'Bedrock Claude 3 Sonnet', 'Bedrock Claude 2', 'Bedrock Claude Instant', 'Bedrock Llama 3', 'Bedrock Mistral Large']
...
def process_file(document, model):
    ...
    with st.spinner(f'File {document} uploaded successfully and starting summarization...'):
        ...
        if document_summary != None:
            st.session_state['document_summary'] = document_summary["content"][0]["text"]
...
def get_answers(input_text, model):
    if input_text is not None and input_text != '' and st.session_state['document_summary']:
        result = shrlib.get_answers(
            st.session_state['document_summary'], input_text, model)
        chat_history = st.session_state['chat_history'] or []
        chat_history.insert(0, "A: " + result)
        chat_history.insert(0, "Q: " + input_text)
