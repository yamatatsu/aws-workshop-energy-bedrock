...
fms = ['Bedrock Titan', 'Bedrock Claude 3.5 Sonnet v2', 'Bedrock Llama 3.1', 'Bedrock Mistral Large 2']
default_model = fms.index('Bedrock Claude 3.5 Sonnet v2')
...
def generate_conditions(model):
  ...
  for i in range(0, len(conditions)):
    with tabs[i]:
      ...
      jsonCode = rcalib.get_answers(st.session_state['motor_summary'], code_text, model)
      st.session_state.generated_code = jsonCode
      st.code(jsonCode,language=str(conditions[i]))
...
def describe_sensor_data(model, input_text):
  ...
  rca = rcalib.get_answers("** motor specs **" + st.session_state['motor_summary']+ "\n", rca_prompt, model)
...
def run_root_cause_analysis(model, rca):
    ...
    rca_prompt = "Based on your observations:\n" + rca + "\n and based the given motor specs, TASK: please describe if there is any anomaly and the potential root cause(s):\n"
    rca = rcalib.get_answers("** motor specs **" + st.session_state['motor_summary']+ "\n", rca_prompt, model)
